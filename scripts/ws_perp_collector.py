"""
WebSocket Perpetual Futures Collector - Real-Time BTC/ETH Perpetual Data
Collects tick data for BTC-PERPETUAL and ETH-PERPETUAL

This is the main WebSocket collector for perpetual futures that subscribes to
BTC-PERPETUAL and ETH-PERPETUAL and streams quote/trade ticks to the database.

Features:
- Subscribe to both BTC-PERPETUAL and ETH-PERPETUAL
- Real-time quotes (book.{instrument}.100ms)
- Real-time trades (trades.{instrument}.100ms)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/ws_perp_collector.log')
    ]
)
logger = logging.getLogger(__name__)


class PerpetualOrderbookSnapshotFetcher:
    """
    Fetch orderbook snapshots via REST API for perpetuals.
    """

    def __init__(self, database_url: str, rest_api_url: str = "https://www.deribit.com/api/v2"):
        self.database_url = database_url
        self.rest_api_url = rest_api_url
        self.writer = PerpetualTickWriter(database_url)
        logger.info("PerpetualOrderbookSnapshotFetcher initialized")
    ...
    async def fetch_and_populate(self, instruments: List[str], save_full_depth: bool = False) -> Dict[str, int]:
        ...
        logger.info(f"Fetching perpetual orderbook snapshots for {len(instruments)} instruments...")
        ...
        logger.info(
            f"✅ Perpetual snapshot complete: {stats['instruments_fetched']} instruments fetched, "
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                ...
                quote = {
                    ...
                    'funding_rate': result.get('funding_8h'),
...
class WebSocketPerpetualCollector:
    """
    Main WebSocket collector for perpetual futures tick data.
    """
    ...
    def __init__(
        ...
        """Initialize WebSocket perpetual collector."""
        self.ws_url = ws_url
        self.database_url = database_url
        self.flush_interval_sec = flush_interval_sec

        # Perpetual instruments (hardcoded)
        ...
        logger.info(
            f"WebSocketPerpetualCollector initialized: "
            f"instruments={self.instruments}, flush_interval={flush_interval_sec}s"
        )

    async def start(self):
        """Start the collector (main entry point)."""
        logger.info("Starting perpetual WebSocket tick collector...")

        try:
            # Connect to database
            await self.writer.connect()

            logger.info(f"Perpetual instruments: {self.instruments}")

            # Fetch initial orderbook snapshot via REST API
            logger.info("Fetching initial perpetual orderbook snapshot via REST API...")
            snapshot_fetcher = PerpetualOrderbookSnapshotFetcher(
            ...
            tasks = [
                ...
                # NOTE: _periodic_snapshot_loop() disabled - ticker channel provides all data
            ]
            ...
            logger.error(f"Fatal error in perpetual collector: {e}", exc_info=True)
            raise
        finally:
            await self.stop()
    ...
    async def stop(self):
        ...
        logger.info("Stopping perpetual WebSocket tick collector...")
        self.running = False
        ...
        logger.info("Perpetual WebSocket tick collector stopped")
        ...
        while self.running:
            try:
                ...
                async with websockets.connect(
                    ...
                    self.ws = ws
                    logger.info("Perpetual WebSocket connected successfully")
    ...
    async def _subscribe_to_instruments(self):
        ...
        for instrument in self.instruments:
            channels.append(f"ticker.{instrument}.100ms")  # Complete ticker (bid/ask/mark/index/funding)
            channels.append(f"trades.{instrument}.100ms")  # Trades
        ...
        logger.info(f"Subscribing to {len(channels)} channels ({len(self.instruments)} perpetuals)...")
    ...
    async def _handle_quote_tick(self, data: Dict):
        """Handle quote tick from ticker.{instrument}.100ms channel.

        Ticker provides complete pricing data (bid/ask/mark/index/funding) in simple format.
        This replaces both book channel (for bid/ask) and REST API snapshots (for mark/index/funding).

        Ticker data format:
        {
            "timestamp": 1234567890,
            "instrument_name": "BTC-PERPETUAL",
            "best_bid_price": 104775.5,
            "best_bid_amount": 42250,
            "best_ask_price": 104776.0,
            "best_ask_amount": 32420,
            "mark_price": 104784.42,
            "index_price": 104632.35,
            "funding_8h": 0.00001526,
        ...
        try:
            quote = {
                ...
                'funding_rate': data.get('funding_8h'),
        ...
        try:
            ...
            for trade_data in trades:
                trade = {
                    ...
                    'tick_direction': trade_data.get('tick_direction'),
                    'liquidation': trade_data.get('liquidation', 'none') != 'none'
                }
            ...
            try:
                ...
                if self.last_tick_time:
                    ...
                    if time_since_last_tick > self.heartbeat_timeout_sec:
                        logger.warning(
                            f"No perpetual ticks received for {time_since_last_tick:.0f}s"
                        )
    ...
    async def _stats_logger(self):
        ...
        while self.running:
            try:
                ...
                logger.info(
                    f"[PERP] STATS | Ticks: {self.stats['ticks_processed']} "
    ...
    async def _periodic_snapshot_loop(self):
        ...
        logger.info(f"Periodic snapshot loop started for perpetuals (interval: {snapshot_interval_sec}s)")
        ...
        while self.running:
            try:
                ...
                logger.info("Fetching periodic perpetual REST API snapshot...")
                ...
                logger.info(
                    f"Periodic perpetual snapshot complete: {snapshot_stats['quotes_populated']} quotes, "
...
async def main():
    """Main entry point for WebSocket perpetual collector."""
    ...
    logger.info("Starting perpetual futures collector for BTC-PERPETUAL and ETH-PERPETUAL")
